...
We will be operating a two-stage submission process with the following key dates.

- **July 25th**: Evaluation data release for HASPI evaluation for all entrants.
- **Aug 14th**: Registration deadline. 

- **Aug 18th**: All entrants submit (i) audio for HASPI evaluation and (ii) a draft of their technical report (details below).
- **Aug 25th**: Deadline by which all entrants must submit two page technical reports to Clarity-2022 workshop (details below). 
- **Aug 25th**: Entrants informed which systems are going forward to the listening test evaluation stage. Evaluation data for listening tests released to those entrants.
- **Sept 1st**: Entrants submit audio for listening tests.
...
<iframe src="https://docs.google.com/forms/d/e/1FAIpQLScsGiTfgjApVBPrAv63KW0MPinVaewBZnnq7X7_ELmqGq1KAw/viewform?embedded=true" width="540" height="300" frameborder="0" marginheight="0" marginwidth="0">Loading…</iframe>
...
For the HASPI evaluation, there will be one listener per scene and the scene-listener mapping will be the same for all teams. For the listening test evaluation, there will be five listeners per scene and each team will have a separate scene-listener mapping. The file formats will be the same as used for the development data; for details see the [CEC2 Data](./cec2_data) page.
## What audio do I need to submit?

You must submit the audio signals produced at the output of your simulated hearing aid for the evaluation datasets. You will be asked to provide two sets of signals: the first for the HASPI evaluation (1st submission - August 18th) and the second for the listening tests (2nd submission - September 1st).

1. **HASPI evaluation**. Signals should be submitted in floating point WAV format with a 44.1 kHz sampling rate. For levels, we will follow the convention in the baseline hearing aid (at the output) and hearing loss models. That is, a +/-1 square wave has RMS = 0 dB FS and corresponds to 120 dB.

2. **Listening tests**. Signals should be submitted as 16-bit WAV files with a 32 kHz sampling rate (due to hardware limitations). You should ensure that any samples that are >+1 or <-1 have been hard-clipped at +/-1 before submission. Here, 0 dB FS corresponds to approximately 100 dB, given the capabilities of the reproduction equipment. These signals will be played as is to the listener panel.
...
Your processed signals should be named using the conventions used by the baseline system, i.e., `<Scene ID>_<Listener ID>_HA-output.wav` and explained on the [CEC2 data page](./cec2_data#d6-enhanced-signals). 
...
**The two page technical report must be submitted as a paper to the Clarity-2022 Workshop. Deadline - see date above**. An author kit and submission instructions are available at the [workshop website](https://claritychallenge.github.io/clarity2021-workshop/submissions.html).

**A draft of the report needs to be uploaded to the Google Drive along  with your HASPI signals by 25th August**. The draft needs to be sufficiently complete for us to judge whether your system is compliant with the challenge rules.
