...
The scenarios have many parameters randomized:
...
- Height H set using a Gaussian distribution random number generator with a mean of 2.7 m and standard deviation of 0.8m.
...
- -W/2+1 <= x_r <= W/2-1
- 1 \le y_r \le L-1
- z_r either 1.2 m1.2m (sitting) or 1.6 m (standing).
...
- The rotation last for 200 ms (standard deviation =10 ms)
