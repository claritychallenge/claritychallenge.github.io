...
The baseline is distributed as part of the pyclarity Python package (`pyclarity` >=0.6.0), which is available on [Github](https://github.com/claritychallenge/clarity). The relevant scripts can be found in the recipes directory, `recipes/cpc2/baseline`. Download the code from Github and follow the instructions in the CPC3 baseline recipes's `README` to run the baseline on the CPC3 dataset.
