...
The results are stored in a CSV file which is read by the report_scores script to generate a report. The code has been designed so that the evaluation can be run on multiple processes and the report_scores will then collate the results from each CSV file.

For examples of how to run the script see the [README.md](https://github.com/claritychallenge/clarity/tree/main/recipes/cec3) file in the CEC3 baseline recipe of the PyClarity toolkit on GitHub.

### Baseline performance for Task 2
