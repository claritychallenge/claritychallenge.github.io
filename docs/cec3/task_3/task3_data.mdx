...
<figcaption>Figure 1  Recording an outdoor impulse response</figcaption>
</figure>
...
The **reference signal** is an anechoic version of the target that is to be used as the standard reference for HASPI evaluation. Note, this signal is an appropriately delayed version of the target signal but it has not been corrected for the HRTF of the listener.
...
[
    {
        ...
        "target": {  // The location of the target -- provides an idea of the distance
            "position": [
                0.9,
                0.0,
                1.6
...
- The `listener' and`target' locations provide an approximately correct listener-target distance but for this task they do not have meaning in an absolute sense. i.e. the listener position is always recorded at (0, 0, 1.6).
